...
export function confirmDelete(title = 'Delete Item?', onConfirm, description = 'This action cannot be undone.') {
  showConfirmModal({
    title,
    description,
    confirmText: 'Delete',
...
export function confirmAction(title = 'Confirm Action?', onConfirm, actionLabel = 'Confirm', description = 'Are you sure you want to proceed?') {
  showConfirmModal({
    title,
    description,
    ...
    <div className="fixed inset-0 z-[99999] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#111827] border border-gray-200 dark:border-gray-800 rounded-2xl w-full max-w-sm p-6 shadow-2xl space-y-4">
        <div className="flex items-start gap-3">
          <div className={`p-3 rounded-2xl flex-shrink-0 ${type === 'danger' ? 'bg-red-500/10 text-red-600 dark:text-red-400 border border-red-500/20' : 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20'}`}>
