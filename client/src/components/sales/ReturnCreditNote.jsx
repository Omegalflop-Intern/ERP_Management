...
export default function ReturnCreditNote({ sale, returnLog, returnLogsGroup }) {
  ...
  return (
    <div
      className="bg-white text-slate-900 p-8 max-w-[210mm] min-h-[260mm] mx-auto flex flex-col justify-between shadow-lg border border-slate-200 print:shadow-none print:border-none print:p-4 print:max-w-none print:w-full print:min-h-0 print:h-auto print:break-inside-avoid"
      ...
      <div>
        ...
        <div className="flex justify-between items-start mb-4 border-b-2 border-slate-900 pb-3">
          <div className="space-y-0.5">
            ...
            {companyInfo.slogan && <p className="text-xs text-slate-600 font-medium">{companyInfo.slogan}</p>}
            {companyInfo.address && <p className="text-xs text-slate-500">{companyInfo.address}</p>}
            <p className="text-xs text-slate-500">Phone: {companyInfo.phone} {companyInfo.binVat && `| ${companyInfo.binVat}`}</p>
          </div>
          ...
          <div className="text-right space-y-1">
            ...
            <div>
              <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Credit Note No</p>
              <p className="text-lg font-mono font-bold text-red-700">{creditNoteNumber}</p>
            </div>
            <div className="text-xs text-slate-600">
              <p><strong>Original Invoice:</strong> <span className="font-mono font-bold text-slate-900">{sale?.invoiceNumber}</span></p>
              <p><strong>Return Date:</strong> {returnDate.toLocaleDateString('en-BD', { month: 'short', day: '2-digit', year: 'numeric' })}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-12 gap-4 mb-4 text-xs">
          <div className="col-span-8 bg-slate-50 p-3 rounded-lg border border-slate-200">
            <span className="font-bold text-slate-400 uppercase text-[9px] block mb-1">Customer / Refund Recipient</span>
            ...
            {sale?.customerAddress && <p className="text-slate-500 text-[11px]">{sale.customerAddress}</p>}
          </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-900 text-white uppercase text-[10px] tracking-wider">
                ...
                <th className="py-2 px-3 text-right w-24 border-r border-slate-800">Refund Price</th>
                <th className="py-2 px-3 text-right w-24">Subtotal</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200">
              {logsToRender.map((log, index) => {
                const effectivePrice = log.effectiveUnitPrice || (log.qty > 0 ? Math.round(log.refundAmount / log.qty) : log.refundAmount);

                return (
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                    <td className="py-2 px-3 text-center font-bold text-slate-500 border-r border-slate-200">{index + 1}</td>
                    <td className="py-2 px-3 border-r border-slate-200">
                      <div className="font-bold text-slate-900">{log.description || 'Returned Line Item'}</div>
                      <div className="text-[10px] text-slate-500">Reason: <span className="font-semibold uppercase text-red-600">{log.reason || 'defective'}</span></div>
                    </td>
                    ...
                    <td className="py-2 px-3 text-center font-bold border-r border-slate-200">{log.qty}</td>
                    <td className="py-2 px-3 text-right border-r border-slate-200">৳{effectivePrice?.toLocaleString()}</td>
                    <td className="py-2 px-3 text-right font-bold text-red-700">৳{log.refundAmount?.toLocaleString()}</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <div className="grid grid-cols-12 gap-4 mb-4">
          <div className="col-span-7 text-xs space-y-2">
            <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-200">
              <span className="font-bold uppercase text-[9px] text-slate-400 block mb-0.5">Refund Amount in Words</span>
              <p className="font-bold text-slate-900 italic text-xs leading-relaxed">{numberToWordsBD(totalRefund)}</p>
            </div>
            <p className="text-[10px] text-slate-500 leading-snug">
              Note: This Credit Note represents an official return voucher adjusting invoice #{sale?.invoiceNumber}. Returned goods have been received intact and processed into inventory.
            </p>
          </div>
          ...
          <div className="col-span-5 text-xs space-y-1 bg-red-50/40 p-3 rounded-lg border border-red-200">
            ...
            <div className="flex justify-between items-center text-sm font-black text-slate-900 py-1.5 border-t border-slate-900 mt-1">
              ...
              <span>৳{Math.max(0, (sale?.netTotal || 0) - (sale?.returnedAmount || totalRefund)).toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
