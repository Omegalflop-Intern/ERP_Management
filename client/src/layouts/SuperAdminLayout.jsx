...
const NAV_ITEMS = [
  ...
  { to: '/super-admin/contacts', icon: MessageSquare, label: 'Website Inquiries' },
  { to: '/super-admin/analytics', icon: BarChart3, label: 'System Analytics' },
...
export default function SuperAdminLayout() {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      <div
        className={`flex items-center gap-3 px-4 py-5 border-b border-slate-200 dark:border-slate-800 ${collapsed ? 'justify-center' : ''}`}
      >
        <div className="w-9 h-9 rounded-xl bg-[#2563EB] flex items-center justify-center flex-shrink-0 shadow-md shadow-blue-600/20">
        ...
        {!collapsed && (
          <div>
            ...
            <div className="text-[10px] text-[#2563EB] dark:text-blue-400 font-bold tracking-wider uppercase">
              Super Admin Panel
            </div>
          </div>
        )}
      ...
      >
        ...
        <div className={`flex items-center ${collapsed ? 'flex-col gap-2' : 'gap-2'}`}>
          <button
            onClick={toggleTheme}
            className="p-2.5 rounded-xl text-slate-800 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors border border-slate-200 dark:border-slate-700"
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3.5 py-2.5 rounded-xl text-xs font-bold bg-red-600 hover:bg-red-700 text-white shadow-sm transition-all ${collapsed ? 'justify-center w-full' : 'flex-1'}`}
            title="Logout"
          >
    ...
    <div className="flex h-screen bg-slate-50 dark:bg-[#08080c] text-slate-900 dark:text-slate-100 font-sans overflow-hidden">
      ...
      >
        ...
        <button
          ...
          className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-full z-10 w-5 h-10 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-white border border-slate-200 dark:border-slate-700 rounded-r-lg flex items-center justify-center transition-colors shadow-md"
        >
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          <div
            ...
            onClick={() => setMobileOpen(false)}
          />
          <aside className="relative z-10 w-64 flex flex-col bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 h-full shadow-2xl">
            <button
              onClick={() => setMobileOpen(false)}
              className="absolute top-4 right-4 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white"
            >
              <X className="w-5 h-5" />
            </button>
            <SidebarContent />
