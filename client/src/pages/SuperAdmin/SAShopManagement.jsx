...
const PLAN_COLORS = {
  FREE: 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300 border border-slate-200 dark:border-slate-700',
  STARTER: 'bg-blue-50 text-blue-700 dark:bg-blue-950/60 dark:text-blue-400 border border-blue-200 dark:border-blue-800/60',
  PRO: 'bg-purple-50 text-purple-700 dark:bg-purple-950/60 dark:text-purple-400 border border-purple-200 dark:border-purple-800/60',
  ENTERPRISE: 'bg-amber-50 text-amber-700 dark:bg-amber-950/60 dark:text-amber-400 border border-amber-200 dark:border-amber-800/60',
};
...
function EditTenantModal({ tenant, onClose, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-3xl border border-slate-200 dark:border-slate-800 shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <Pencil className="w-4 h-4 text-orange-500" /> Edit Shop — {tenant.shopName}
          </h3>
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"
...
export default function SAShopManagement() {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!body.username || !body.username.trim()) delete body.username;
  ...
  const handleDownloadBackup = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `omnimanage_system_backup_${new Date().toISOString().split('T')[0]}.json`;
      a.click();
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async (ids) => api.delete('/tenants/bulk', { data: { ids } }),
    onSuccess: (res) => {
      toast.success(res.data?.message || 'Selected shops deleted');
      setSelectedTenants([]);
      qc.invalidateQueries({ queryKey: ['sa-shops'] });
      qc.invalidateQueries({ queryKey: ['sa-stats'] });
    },
    onError: (e) => toast.error(e.response?.data?.message || 'Bulk delete failed'),
  });

  const handleDelete = async (t) => {
    const confirmed = await confirmDelete(
      `Delete "${t.shopName}"? This will permanently remove all associated data.`
    );
  ...
  const handleBulkDelete = async () => {
    ...
    const confirmed = await confirmDelete(
      `Delete ${selectedTenants.length} selected shop(s)? All shop data will be removed.`
    );
    if (confirmed) bulkDeleteMutation.mutate(selectedTenants);
  };

  const toggleSelectAll = () => {
    if (selectedTenants.length === tenants.length && tenants.length > 0) {
      setSelectedTenants([]);
    } else {
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 shadow-sm">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Provision tenants, adjust subdomains, grant temporary support access, and enforce subscription plans
            </p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2.5">
          {tenants.length > 0 && (
            <button
              ...
              className="flex items-center gap-2 px-3.5 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold rounded-2xl transition-all border border-slate-200 dark:border-slate-700"
            >
              <input
                type="checkbox"
              ...
              />
              <span>{selectedTenants.length === tenants.length && tenants.length > 0 ? 'Deselect All' : 'Select All'}</span>
            </button>
          )}
          ...
          >
            ...
            <span>{backupLoading ? 'Exporting...' : 'System Backup'}</span>
          </button>
          ...
          {selectedTenants.length > 0 && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white text-xs font-black uppercase tracking-wider rounded-2xl transition-all shadow-sm shadow-red-600/20"
            >
              <Trash2 className="w-3.5 h-3.5" /> Bulk Delete ({selectedTenants.length})
            </button>
          )}
          <button
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search shops by store name, owner, email, subdomain..."
          className="w-full pl-11 pr-4 py-3 text-xs border border-slate-200/80 dark:border-slate-800/80 rounded-2xl bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm font-medium"
        />
      </div>
      ...
      {isLoading ? (
        ...
        <div className="py-20 text-center bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 max-w-md mx-auto shadow-sm">
          ...
          <h3 className="font-bold text-slate-700 dark:text-slate-300 text-sm">No shops found</h3>
          <p className="text-xs text-slate-400 mt-1">
            Create a tenant shop using the button above
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {tenants.map((t) => (
            <div
              key={t._id}
              className={`bg-white dark:bg-slate-900 border rounded-2xl p-5 shadow-sm hover:shadow-md transition-all flex flex-col gap-3 ${
            ...
            >
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2.5 min-w-0">
                  <input
                    ...
                    className="w-4 h-4 rounded text-indigo-600 focus:ring-indigo-500 cursor-pointer flex-shrink-0"
                  />
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      <span
                        className={`text-[10px] px-2 py-0.5 rounded-full font-bold border ${STATUS_COLORS[t.status] || ''}`}
                      >
                        {t.status}
                      </span>
                      <span
                        className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${PLAN_COLORS[t.plan] || ''}`}
                      >
                        {t.plan}
                      </span>
                    </div>
              ...
              <div className="grid grid-cols-2 gap-2 p-2.5 bg-slate-50 dark:bg-slate-800/60 rounded-xl text-xs border border-slate-100 dark:border-slate-800">
                ...
                <div className="col-span-2 flex items-center gap-1.5">
                  ...
                  <strong
                    className={
                      t.expiresAt ? 'text-amber-600 dark:text-amber-400' : 'text-slate-400'
                    }
                  >
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="bg-blue-50 dark:bg-blue-950/20 rounded-lg p-2">
                    ...
                    <div className="font-bold text-slate-800 dark:text-white">
                      {t.stats.totalSales} orders
                    </div>
                  </div>
                  <div className="bg-violet-50 dark:bg-violet-950/20 rounded-lg p-2">
                    <div className="text-[10px] text-violet-500 font-semibold uppercase">
                      Revenue
                    </div>
                    <div className="font-bold text-slate-800 dark:text-white">
      ...
      {/* KYC Modal */}
      {kycTenant && <DocumentVaultModal tenant={kycTenant} onClose={() => setKycTenant(null)} />}
