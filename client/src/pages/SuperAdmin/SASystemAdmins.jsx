...
const Avatar = ({ name, src, size = 'md' }) => {
  ...
  return (
    <div
      className={`${sz} ${color} rounded-xl flex items-center justify-center font-bold text-white flex-shrink-0`}
    >
      {initials}
    </div>
  );
...
const Badge = ({ active }) => (
  <span
    className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-400' : 'bg-red-100 text-red-600 dark:bg-red-950/50 dark:text-red-400'}`}
  >
...
export default function SASystemAdmins() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setCreateOpen(false);
      setForm({ username: '', email: '', phone: '', fullName: '', password: '' });
      invalidate();
    },
  ...
  return (
    <div className="space-y-6">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-800">
                  <th className="text-left text-xs font-bold text-slate-500 dark:text-slate-400 px-5 py-3.5">
                    Admin
                  </th>
                  <th className="text-left text-xs font-bold text-slate-500 dark:text-slate-400 px-4 py-3.5 hidden md:table-cell">
                    Contact
                  </th>
                  <th className="text-left text-xs font-bold text-slate-500 dark:text-slate-400 px-4 py-3.5 hidden sm:table-cell">
                    Status
                  </th>
                  <th className="text-left text-xs font-bold text-slate-500 dark:text-slate-400 px-4 py-3.5 hidden lg:table-cell">
                    Created
                  </th>
                  <th className="text-right text-xs font-bold text-slate-500 dark:text-slate-400 px-5 py-3.5">
                    Actions
                  </th>
                </tr>
              </thead>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1">
                            <ShieldCheck className="w-3.5 h-3.5 text-blue-500" />@{admin.username}
                          </div>
                        </div>
                      </div>
                    </td>
        ...
        >
          ...
          <div className="flex gap-3 pt-2">
            <button
              ...
              className="flex-1 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all"
            >
              Cancel
            </button>
            <button
      ...
      <Modal
        ...
        title={`Edit — @${editOpen?.username}`}
      >
        <form
          onSubmit={(e) => {
            e.preventDefault();
            updateMutation.mutate({ id: editOpen.id, body: editForm });
          }}
          className="space-y-4"
        >
          <InputField
