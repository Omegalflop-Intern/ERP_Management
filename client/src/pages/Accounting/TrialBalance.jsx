      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            All accounts with non-zero balances — debits must equal credits
          </p>
        </div>
        <button
          onClick={() => refetch()}
      ...
      {isLoading ? (
        ...
        <div className={cardClass}>
          ...
          <div className="mt-4 flex justify-end">
            {data.balanced ? (
              <span className="px-4 py-1.5 bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400 rounded-full text-sm font-bold">
                Trial Balance is Balanced
              </span>
            ) : (
              <span className="px-4 py-1.5 bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400 rounded-full text-sm font-bold">
                Trial Balance is UNBALANCED — Difference: ৳
                {Math.abs(data.totalDebit - data.totalCredit).toLocaleString()}
              </span>
            )}
          </div>
        </div>
