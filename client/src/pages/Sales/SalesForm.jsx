...
import { Search, Trash2, ShoppingCart, CreditCard, Smartphone, Banknote, RefreshCw, Percent, UserPlus, Plus, X, Package, Wand2, Gift, Tag } from 'lucide-react';
...
export default function SalesForm() {
  ...
  const { data: imeiResults, isLoading: searchingImei } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/inventory', { params: { search: searchQuery, status: 'Available', limit: 20 } });
      return data?.data || [];
    },
  ...
  const { data: productResults, isLoading: searchingProducts } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get('/products', { params: { search: searchQuery || '', limit: 20 } });
  ...
  const imeiProductIds = new Set((imeiResults || []).map(u => u.productId?._id?.toString() || u.productId?.toString()));
  const bulkProducts = (productResults || []).filter(p => !imeiProductIds.has(p._id?.toString()));
  const searchResults = [...(imeiResults || []), ...bulkProducts.map(p => ({
    _id: p._id,
    isBulk: true,
    productId: p,
    imeiOrSerial: '',
  ...
  const { data: customerSearch } = useQuery({
    ...
    enabled: (isCustomerFocused || customerPhone.length >= 2 || customerName.length >= 2) && !customerId,
  });
  ...
  const addToCart = (item) => {
    ...
    if (item.isBulk) {
      ...
      if (existing) {
        ...
        setCart(cart.map(c =>
          c.productId === (item.productId?._id || item.productId)
            ? { ...c, qty: c.qty + 1 }
            : c
        ));
        setSearchQuery('');
  ...
  const paidAmount = (Number(payment.cash) || 0) + (Number(payment.bkash) || 0) + (Number(payment.rocket) || 0) + (Number(payment.nagad) || 0) + (Number(payment.bank) || 0);
  ...
  const updateCartPricesForCustomer = (customerObj) => {
    ...
    setCart((prevCart) =>
      prevCart.map((item) => {
        ...
        const targetPrice = (isB2B && item.wholesalePrice > 0) ? item.wholesalePrice : item.retailPrice;
  ...
  const handleCheckout = () => {
    ...
    if (dueAmount > 0 && !customerPhone) return toast.error('Customer phone required for due amount');
    ...
    checkoutMutation.mutate({
      ...
      items: cart.map(c => ({ productId: c.productId, imeiOrSerial: c.imeiOrSerial, description: c.description, qty: c.qty, unitPrice: c.unitPrice })),
  ...
  const cardCls = styled ? 'neu-card rounded-xl' : 'bg-white dark:bg-[#111827] rounded-xl border border-gray-200 dark:border-gray-800';

  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">New Sale</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400">Scan IMEI or search products to add to cart</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            {(isFocused || searchQuery) && (
              <div className="mt-2 border border-gray-200 dark:border-gray-700 rounded-xl max-h-64 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-[#111827] shadow-lg">
                <div className="px-3 py-1.5 text-[10px] font-bold text-gray-400 uppercase tracking-wider bg-gray-50/50 dark:bg-gray-900/50 flex justify-between items-center">
                  ...
                  <button onClick={() => setIsFocused(false)} className="hover:text-gray-600 dark:hover:text-gray-200">Close</button>
                </div>
                {searching ? (
                  <div className="p-4 text-center text-xs text-gray-400 animate-pulse">Searching products...</div>
                  ...
                  searchResults.map((item) => {
                    ...
                    return (
                      <button key={item._id + (item.imeiOrSerial || 'bulk')} onClick={() => { addToCart(item); setIsFocused(false); }}
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-500 font-mono flex items-center gap-2">
                            {item.isBulk ? (
                              <span className="text-[10px] px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-400 font-bold">Stock Item</span>
                            ) : (
                              <span>IMEI: {item.imeiOrSerial}</span>
                            )}
                            {item.warrantyMonths && <span className="text-[10px] px-1.5 py-0.5 rounded bg-indigo-500/10 text-indigo-400">{item.warrantyMonths}m Warranty</span>}
                          </div>
                        </div>
                        <div className="text-right flex flex-col items-end gap-1">
                          ...
                          <span className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-bold border border-emerald-500/20">
                            <Package className="w-3 h-3 text-emerald-500" /> {item.isBulk ? (item.stockQuantity || item.productId?.stockQuantity || 0) : (item.productStockCount || 1)} in stock
                          </span>
                        </div>
                      </button>
                    );
                  ...
                  <div className="p-6 text-center text-xs text-gray-400">
                    No products found matching "<span className="font-semibold text-gray-300">{searchQuery}</span>"
                  </div>
                )}
              </div>
            )}
          ...
          <div className={`${cardCls} overflow-hidden`}>
            <div className={`px-4 py-3 flex items-center justify-between ${styled ? '' : 'border-b border-gray-200 dark:border-gray-800'}`}>
              <h3 className="font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-2">
                <ShoppingCart className="w-4 h-4" /> Cart ({cart.length})
              </h3>
              <button onClick={addManualItem} className="text-xs text-red-600 dark:text-red-400 hover:underline">+ Add manually</button>
            </div>
            ...
            {cart.length > 0 && (
              <div className="mx-4 my-2 p-2.5 bg-gradient-to-r from-amber-500/15 to-purple-500/15 border border-amber-500/30 rounded-xl flex items-center gap-2.5">
                ...
                <div className="text-xs text-gray-800 dark:text-gray-200">
                  <span className="font-bold text-amber-600 dark:text-amber-400">🎁 Free Gift Promo Reminder:</span>
                  <span className="ml-1">Remind cashier to offer free screen protector, back cover or gift box for device purchase!</span>
                </div>
              </div>
            )}
                ...
                {cart.map((item, i) => (
                  <div key={i} className="px-4 py-3 flex items-center gap-3">
                    <div className="flex-1 min-w-0">
                      {item.isManual ? (
                        <input value={item.description} onChange={(e) => updateCartItem(i, 'description', e.target.value)} placeholder="Product description"
                          className={`w-full px-2 py-1 ${inputCls}`} />
                      ) : (
                        <div className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{item.description}</div>
                      )}
                      <div className="text-xs text-gray-500 font-mono">{item.imeiOrSerial || 'No IMEI'}</div>
                    </div>
                    <div className="w-16">
                      <input type="number" value={item.qty}
                        onFocus={(e) => e.target.select()}
                        onChange={(e) => updateCartItem(i, 'qty', Number(e.target.value))} min={1}
                        className={`w-full px-2 py-1 text-center ${inputCls}`} />
                    </div>
                    <div className="w-28">
                      <input type="number" value={item.unitPrice}
                        onFocus={(e) => e.target.select()}
                        onChange={(e) => updateCartItem(i, 'unitPrice', Number(e.target.value))} min={0}
                        className={`w-full px-2 py-1 text-right ${inputCls}`} />
                    </div>
                    <div className="w-24 text-right text-sm font-medium text-gray-900 dark:text-gray-100">৳{(item.unitPrice * item.qty).toLocaleString()}</div>
                    <button onClick={() => removeFromCart(i)} className="p-1 text-gray-400 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="space-y-4">
          ...
          <div className={`${cardCls} p-4 space-y-3 relative z-30`}>
            <div className="flex items-center justify-between">
              <h3 className="font-semibold text-sm text-gray-900 dark:text-gray-100">Customer Info</h3>
              {customerId ? (
                <button onClick={clearCustomer} className="text-xs text-red-500 hover:underline flex items-center gap-1"><X className="w-3 h-3" /> Clear</button>
              ) : (
                <button onClick={() => setShowCustomerCreate(true)} className="text-xs text-blue-500 hover:underline flex items-center gap-1"><UserPlus className="w-3 h-3" /> New</button>
              )}
            </div>
            ...
            {customerId ? (
              <div className="px-3.5 py-2.5 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-500/30 space-y-1">
                <div className="flex items-center justify-between gap-2">
                  <div className="text-sm font-bold text-green-800 dark:text-green-300 flex items-center gap-1.5">
                    ...
                    {selectedCustomerObj?.companyName && (
                      <span className="text-xs font-normal text-green-700 dark:text-green-400">({selectedCustomerObj.companyName})</span>
                    )}
                  </div>
                ...
                <div className="text-xs text-green-600 dark:text-green-400 font-mono">{customerPhone}</div>
                {customerEmail && <div className="text-xs text-green-600 dark:text-green-400">{customerEmail}</div>}
                {customerAddress && <div className="text-xs text-green-600 dark:text-green-400">{customerAddress}</div>}
              ...
              <div className="space-y-2 relative">
                <input 
                  ...
                  onChange={(e) => { setCustomerName(e.target.value); setIsCustomerFocused(true); }}
                ...
                <div className="relative">
                  <input 
                    ...
                    value={customerPhone} 
                    onFocus={() => setIsCustomerFocused(true)}
                    onChange={(e) => { setCustomerPhone(e.target.value); setCustomerId(null); setIsCustomerFocused(true); }}
                    className={`w-full ${inputCls}`} 
                  />
                  ...
                  {isCustomerFocused && customerSearch?.length > 0 && (
                    <div className="absolute z-50 top-full mt-1 left-0 right-0 bg-white dark:bg-[#111827] border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl max-h-60 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-800">
                      <div className="px-3 py-1.5 text-[10px] font-bold text-gray-400 uppercase tracking-wider bg-gray-50/90 dark:bg-gray-900/90 flex justify-between items-center sticky top-0 backdrop-blur-md z-10">
                        ...
                        <button onClick={() => setIsCustomerFocused(false)} className="hover:text-gray-600 dark:hover:text-gray-200 text-xs">Close</button>
                      </div>
                      {customerSearch.map((c) => {
                        ...
                        return (
                          <button
                            key={c._id}
                            onClick={() => { selectCustomer(c); setIsCustomerFocused(false); }}
                ...
                <input type="email" placeholder="Email (optional)" value={customerEmail} onChange={(e) => setCustomerEmail(e.target.value)}
                  className={`w-full ${inputCls}`} />
                <input type="text" placeholder="Address (optional)" value={customerAddress} onChange={(e) => setCustomerAddress(e.target.value)}
                  className={`w-full ${inputCls}`} />
              </div>
            )}
          </div>
          ...
          <div className={`${cardCls} p-4 space-y-3`}>
            <h3 className="font-semibold text-sm text-gray-900 dark:text-gray-100 flex items-center justify-between">
              ...
              <span className="text-xs font-normal text-amber-500 flex items-center gap-1"><Tag className="w-3 h-3" /> Promo / Gift Cards</span>
            </h3>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-gray-500 uppercase">Discount (৳)</label>
                <input type="number" value={discount} placeholder="0" min={0}
                  onFocus={(e) => e.target.select()}
                  onChange={(e) => setDiscount(e.target.value)}
                  className={`w-full ${inputCls}`} />
              </div>
              <div>
                <label className="text-xs text-gray-500 uppercase flex items-center gap-1"><Percent className="w-3 h-3" /> VAT %</label>
                <input type="number" value={vatRate} placeholder="0" min={0} max={100}
                  onFocus={(e) => e.target.select()}
            ...
            <div>
              <label className="text-xs text-gray-500 uppercase flex items-center gap-1"><Tag className="w-3 h-3 text-indigo-500" /> Promo Code / Gift Card</label>
              <input
                type="text"
          ...
          <div className={`${cardCls} p-4 space-y-2`}>
            <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400"><span>Subtotal</span><span>৳{subTotal.toLocaleString()}</span></div>
            {discountAmount > 0 && <div className="flex justify-between text-sm text-red-600"><span>Discount</span><span>-৳{discountAmount.toLocaleString()}</span></div>}
            {vatAmount > 0 && <div className="flex justify-between text-sm text-blue-600"><span>VAT ({vatRate}%)</span><span>+৳{Math.round(vatAmount).toLocaleString()}</span></div>}
            <div className="flex justify-between text-lg font-bold text-gray-900 dark:text-gray-100 border-t border-gray-200 dark:border-gray-800 pt-2"><span>Total</span><span>৳{Math.round(netTotal).toLocaleString()}</span></div>
            <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400"><span>Paid</span><span>৳{paidAmount.toLocaleString()}</span></div>
            {dueAmount > 0 && <div className="flex justify-between text-sm font-bold text-red-600"><span>Due</span><span>৳{Math.round(dueAmount).toLocaleString()}</span></div>}
          </div>
          ...
          <button onClick={handleCheckout} disabled={checkoutMutation.isPending || cart.length === 0}
            ...
            {checkoutMutation.isPending ? <RefreshCw className="w-4 h-4 animate-spin" /> : 'Complete Sale'}
          </button>
        </div>
      </div>
...
function InlineCustomerModal({ defaultName, defaultPhone, onClose, onCreate, isPending, styled, inputCls }) {
  ...
  const cardCls = styled ? 'neu-card rounded-2xl' : 'bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl';
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className={`${cardCls} w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto`}>
        ...
        <form onSubmit={(e) => { e.preventDefault(); onCreate(form); }} className="p-6 space-y-3 text-sm">
          <div>
            <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Customer Type</label>
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Full Name / Contact Person *</label>
            <input required value={form.name} onChange={(e) => setForm({...form, name: e.target.value})} className={`w-full ${inputCls}`} />
          </div>

          {form.customerType === 'B2B' && (
            <>
              <div>
                <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Company / Shop Name</label>
                <input value={form.companyName} onChange={(e) => setForm({...form, companyName: e.target.value})} placeholder="e.g. M/S Rahat Telecom" className={`w-full ${inputCls}`} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Trade License / BIN / Tax ID</label>
                <input value={form.binOrTaxId} onChange={(e) => setForm({...form, binOrTaxId: e.target.value})} placeholder="e.g. BIN-0098483-2" className={`w-full ${inputCls}`} />
              </div>
            </>
          )}
          ...
          <div>
            ...
            <input required value={form.phone} onChange={(e) => setForm({...form, phone: e.target.value})} className={`w-full ${inputCls} font-mono`} />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={form.address} onChange={(e) => setForm({...form, address: e.target.value})} className={`w-full ${inputCls}`} />
          </div>
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={isPending} className="flex-1 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-medium rounded-lg text-sm transition-colors">
...
function InlineProductModal({ onClose, onCreate, isPending, styled, inputCls }) {
  const [form, setForm] = useState({ name: '', brand: '', category: 'SMARTPHONE', sku: '', costPrice: '', sellingPrice: '', imeiOrSerial: '', ram: '', storage: '', color: '' });
  const cardCls = styled ? 'neu-card rounded-2xl' : 'bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl';
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className={`${cardCls} w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl`}>
        ...
        <form onSubmit={(e) => {
          e.preventDefault();
          onCreate({ ...form, costPrice: Number(form.costPrice), sellingPrice: Number(form.sellingPrice), minStockAlert: 2 });
        }} className="p-6 space-y-3 text-sm">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Product Name *</label>
              <input required value={form.name} onChange={(e) => setForm({...form, name: e.target.value})} className={`w-full ${inputCls}`} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Brand *</label>
              <input required value={form.brand} onChange={(e) => {
                const brandVal = e.target.value;
                setForm(prev => {
                  const updated = { ...prev, brand: brandVal };
                  if (!prev.sku) {
                    const prefix = brandVal.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 8) || 'PROD';
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input required value={form.category} onChange={(e) => setForm({...form, category: e.target.value})} list="pos-cat-list" placeholder="Type or select" className={`w-full ${inputCls}`} />
              <datalist id="pos-cat-list">{(catList || []).map(c => <option key={c._id} value={c.name} />)}</datalist>
            </div>
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">SKU</label>
                <button type="button" onClick={() => {
                  const prefix = (form.brand || 'PROD').toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 8) || 'PROD';
                  const random6 = Math.floor(100000 + Math.random() * 900000);
              ...
              <input value={form.sku} onChange={(e) => setForm({...form, sku: e.target.value.toUpperCase()})} placeholder="Auto if empty" className={`w-full ${inputCls} font-mono`} />
            </div>
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Device IMEI / Serial (Optional)</label>
            <input value={form.imeiOrSerial} onChange={(e) => setForm({...form, imeiOrSerial: e.target.value})} placeholder="15-digit IMEI number" className={`w-full ${inputCls} font-mono`} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Cost Price (৳) *</label>
              <input type="number" required min="0" value={form.costPrice} onChange={(e) => setForm({...form, costPrice: e.target.value})} className={`w-full ${inputCls}`} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Selling Price (৳) *</label>
              <input type="number" required min="0" value={form.sellingPrice} onChange={(e) => setForm({...form, sellingPrice: e.target.value})} className={`w-full ${inputCls}`} />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">RAM</label>
              <input value={form.ram} onChange={(e) => setForm({...form, ram: e.target.value})} placeholder="8GB" className={`w-full ${inputCls}`} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Storage</label>
              <input value={form.storage} onChange={(e) => setForm({...form, storage: e.target.value})} placeholder="256GB" className={`w-full ${inputCls}`} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Color</label>
              <input value={form.color} onChange={(e) => setForm({...form, color: e.target.value})} className={`w-full ${inputCls}`} />
            </div>
          </div>
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 font-medium rounded-lg text-sm transition-colors">Cancel</button>
            <button type="submit" disabled={isPending} className="flex-1 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white font-medium rounded-lg text-sm transition-colors">
