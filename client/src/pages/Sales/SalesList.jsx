...
export default function SalesList() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Sale deleted'); queryClient.invalidateQueries(['sales']); },
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">View all transactions and invoices</p>
        </div>
        <button onClick={() => navigate('/sales/new')} className="flex items-center gap-2 px-4 py-2 bg-red-700 hover:bg-red-600 text-white font-medium rounded-lg text-sm transition-all">
          ...
          <table className="w-full">
            ...
            <tbody>
              {isLoading ? (
                ...
                sales.map((s) => {
                  const rawPaid = (s.paymentBreakdown?.cash || 0) + (s.paymentBreakdown?.bkash || 0) + (s.paymentBreakdown?.rocket || 0) + (s.paymentBreakdown?.nagad || 0) + (s.paymentBreakdown?.bank || 0);
                  const netPaid = Math.max(0, rawPaid - (s.returnedAmount || 0));
                  const isWholesale = s.saleType === 'WHOLESALE' || s.customerId?.customerType === 'B2B';
                  return (
                    <tr key={s._id} className="border-b border-gray-100 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/30">
                      ...
                      <td className="px-4 py-3">
                        {s.status === 'RETURNED' ? (
                          <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold bg-red-500/10 text-red-600 dark:text-red-400 border border-red-500/20">Returned</span>
                        ) : s.status === 'PARTIALLY_RETURNED' ? (
                          <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20">Partial Ret.</span>
                        ) : (
                          <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">Completed</span>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        <div className="text-sm text-gray-900 dark:text-gray-100">{s.customerName}</div>
                        <div className="text-xs text-gray-500">{s.customerPhone}</div>
                      </td>
                      <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{s.lineItems?.length || 0}</td>
                      <td className="px-4 py-3 text-right text-sm font-medium text-gray-900 dark:text-gray-100">
                        <div>৳{s.netTotal?.toLocaleString()}</div>
                        {(s.returnedAmount > 0 || s.returnLogs?.length > 0) && (
                          <div className="text-[10px] text-red-600 dark:text-red-400 font-bold mt-0.5" title="Refunded amount & item returns">
                            ↩ Ref: ৳{(s.returnedAmount || 0).toLocaleString()}
                          </div>
                        )}
                      </td>
                      <td className="px-4 py-3 text-right text-sm font-medium">
                        <div className="text-emerald-600 dark:text-emerald-400 font-bold">৳{netPaid.toLocaleString()}</div>
                        {s.returnedAmount > 0 && (
                          <div className="text-[9px] text-gray-400 line-through">Rec'd: ৳{rawPaid.toLocaleString()}</div>
                        )}
                      </td>
                      <td className="px-4 py-3 text-right text-sm font-medium">
                        {s.paymentBreakdown?.dueAmount > 0 ? (
                          <span className="text-red-600 dark:text-red-400">৳{s.paymentBreakdown.dueAmount?.toLocaleString()}</span>
                        ) : (
                          <span className="text-gray-400">—</span>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex gap-1 flex-wrap">
                          ...
                          {s.paymentBreakdown?.rocket > 0 && <span className="text-[10px] px-1.5 py-0.5 rounded bg-purple-100 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400">Rocket</span>}
                          {s.paymentBreakdown?.nagad > 0 && <span className="text-[10px] px-1.5 py-0.5 rounded bg-yellow-100 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400">Nagad</span>}
                          {s.paymentBreakdown?.bank > 0 && <span className="text-[10px] px-1.5 py-0.5 rounded bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400">Bank</span>}
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-2">
                          <button onClick={() => navigate(`/sales/${s._id}`)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500 hover:text-blue-600 transition-colors" title="View / Print / Return"><Eye className="w-4 h-4" /></button>
                        </div>
                      </td>
                    </tr>
                  );
