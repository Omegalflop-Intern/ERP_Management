...
export default function DueCollection() {
  ...
  const summaryCards = [
    ...
    },
    {
      ...
      value: new Set(sales.map((s) => s.customerPhone).filter(Boolean)).size,
      color: 'text-amber-600 dark:text-amber-400',
      icon: User,
    },
    {
      label: 'Invoices with Due',
      value: sales.length,
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (method === 'cash') updatedBreakdown.cash = (updatedBreakdown.cash || 0) + collectAmount;
        ...
        updatedBreakdown.rocket = (updatedBreakdown.rocket || 0) + collectAmount;
      else if (method === 'nagad')
        updatedBreakdown.nagad = (updatedBreakdown.nagad || 0) + collectAmount;
      else if (method === 'bank')
        ...
        <div className="p-6 space-y-4">
          <div className="bg-gray-50 dark:bg-gray-900 rounded-xl p-4">
            ...
            <div className="text-sm text-gray-700 dark:text-gray-300 mt-1">
              {sale.customerName} &middot; {sale.customerPhone}
            </div>
            <div className="text-lg font-bold text-red-600 dark:text-red-400 mt-2">
