...
import { Settings as SettingsIcon, Save, Building2, DollarSign, Package, FileText, Globe, Upload, Image as ImageIcon, Database, Download, RefreshCw, AlertTriangle } from 'lucide-react';
import api, { getAssetUrl } from '../../lib/api';
import { useTheme } from '../../context/ThemeContext';

const settingGroups = [
  { key: 'company', label: 'Company Settings', icon: Building2, keys: ['companyName', 'companySlogan', 'companyAddress', 'companyPhone', 'companyEmail', 'binVat', 'companyLogo'] },
  { key: 'finance', label: 'Finance & Tax', icon: DollarSign, keys: ['currency', 'currencySymbol', 'defaultVatRate', 'taxEnabled'] },
  { key: 'inventory', label: 'Inventory', icon: Package, keys: ['lowStockThreshold', 'autoReorderEnabled'] },
...
const fieldLabels = {
  companyName: 'Company Name', companySlogan: 'Company Tagline / Slogan', companyAddress: 'Company Address', companyPhone: 'Company Phone',
  companyEmail: 'Company Email', binVat: 'BIN / VAT Reg No', companyLogo: 'Company Logo', currency: 'Currency', currencySymbol: 'Currency Symbol',
  defaultVatRate: 'Default VAT Rate (%)', taxEnabled: 'Tax Enabled', lowStockThreshold: 'Low Stock Threshold',
  autoReorderEnabled: 'Auto Reorder', defaultWarrantyMonths: 'Default Warranty (Months)',
  invoiceFooter: 'Invoice Footer Text', timezone: 'Timezone', dateFormat: 'Date Format',
};
...
export default function SettingsPage() {
  ...
  const handleRestoreFile = (e) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (!window.confirm('WARNING: Restoring will overwrite current database records with backup data. Are you sure?')) {
          return;
        }
  ...
  const inputCls = styled ? 'neu-input w-full px-3 py-2.5 rounded-xl text-sm' : 'w-full px-3 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 text-sm focus:ring-2 focus:ring-red-500 outline-none';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">Manage your ERP configuration and database backups</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className={`${cardCls} p-6 lg:col-span-3`}>
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">{activeGroupData?.label}</h2>
          ...
          {activeGroup === 'backup' ? (
            <div className="space-y-6">
              ...
              <div className="flex items-start gap-3 p-3 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
                ...
                <div>
                  <p className="text-xs font-bold text-blue-700 dark:text-blue-400">Weekly Auto-Backup Active</p>
                  <p className="text-xs text-blue-600 dark:text-blue-500 mt-0.5">The server automatically saves a backup to <code className="bg-blue-100 dark:bg-blue-900/40 px-1 rounded">server/backups/</code> once every 7 days. Up to 8 weekly backups are kept. Use the button below to trigger one immediately.</p>
                </div>
              </div>
              ...
              <div className="p-4 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 space-y-3">
                ...
                <p className="text-xs text-gray-500">Saves a timestamped backup JSON file to <code className="bg-gray-100 dark:bg-gray-800 px-1 rounded">server/backups/</code> immediately — same as the weekly auto-backup but triggered on demand.</p>
                {lastServerBackup && (
                  <p className="text-[11px] text-emerald-600 dark:text-emerald-400 font-mono">Last saved: {lastServerBackup}</p>
                )}
                <button
              ...
              <div className="p-4 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 space-y-3">
                ...
                <p className="text-xs text-gray-500">Download a complete JSON backup of all collections directly to your computer for offline storage.</p>
                <button
              ...
              <div className="p-4 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 space-y-3">
                ...
                <div className="flex items-center gap-2 text-amber-600 dark:text-amber-400 text-xs font-semibold">
                  <AlertTriangle className="w-4 h-4 shrink-0" /> Restoring will overwrite existing collection documents with the backup file data.
                </div>
            ...
            <div className="space-y-4 animate-pulse">
              {Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-12 bg-gray-200 dark:bg-gray-700 rounded-xl" />)}
            </div>
          ) : (
            <div className="space-y-4">
              {activeGroupData?.keys.map(key => (
                <div key={key}>
                  <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">{fieldLabels[key] || key}</label>
                  {key === 'companyLogo' ? (
                    <div className="flex items-center gap-4 border border-dashed border-gray-300 dark:border-gray-700 rounded-xl p-4 bg-gray-50 dark:bg-gray-800/40">
                      {form.companyLogo ? (
                        <img src={getAssetUrl(form.companyLogo)} alt="Company Logo" className="h-16 w-auto object-contain rounded border border-gray-200 dark:border-gray-700 bg-white p-1" />
                      ) : (
                      ...
                      <div className="space-y-1">
                        <label className="inline-flex items-center gap-2 px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-semibold cursor-pointer transition-colors">
                          <Upload className="w-4 h-4" /> {logoMutation.isPending ? 'Uploading...' : 'Upload Logo Image'}
                          <input type="file" accept="image/*" onChange={handleFileChange} className="hidden" disabled={logoMutation.isPending} />
                        </label>
                        <p className="text-[11px] text-gray-500">Supported formats: PNG, JPG, WEBP (Max 5MB). Saved to <code className="text-red-600 font-mono">uploads/logos</code>.</p>
                      </div>
                    </div>
                  ) : key === 'taxEnabled' || key === 'autoReorderEnabled' ? (
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" checked={!!form[key]} onChange={e => setForm({ ...form, [key]: e.target.checked })} className="rounded border-gray-300 text-red-600 focus:ring-red-500" />
                      <span className="text-sm text-gray-700 dark:text-gray-300">{form[key] ? 'Enabled' : 'Disabled'}</span>
                    </label>
                  ) : key === 'defaultVatRate' || key === 'lowStockThreshold' || key === 'defaultWarrantyMonths' ? (
                    <input type="number" value={form[key] || ''} onChange={e => setForm({ ...form, [key]: Number(e.target.value) })} className={inputCls} />
                  ) : key === 'invoiceFooter' ? (
                    <textarea value={form[key] || ''} onChange={e => setForm({ ...form, [key]: e.target.value })} className={inputCls} rows={3} />
                  ) : (
