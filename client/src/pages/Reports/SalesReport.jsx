...
import { Calendar, TrendingUp, DollarSign, ShoppingCart, BarChart3, Download, RotateCcw } from 'lucide-react';
...
export default function SalesReport() {
  ...
  const stats = useMemo(() => {
    ...
    const totalRevenue = sales.reduce((sum, s) => sum + ((s.netTotal || 0) - (s.returnedAmount || 0)), 0);
    ...
    const retailSales = sales.filter(s => s.saleType === 'RETAIL' || (!s.saleType && s.customerId?.customerType !== 'B2B'));
    const wholesaleSales = sales.filter(s => s.saleType === 'WHOLESALE' || s.customerId?.customerType === 'B2B');

    const retailRevenue = retailSales.reduce((sum, s) => sum + ((s.netTotal || 0) - (s.returnedAmount || 0)), 0);
    const wholesaleRevenue = wholesaleSales.reduce((sum, s) => sum + ((s.netTotal || 0) - (s.returnedAmount || 0)), 0);

    const totalPaidRaw = sales.reduce((sum, s) => {
      return sum + (s.paymentBreakdown?.cash || 0) + (s.paymentBreakdown?.bkash || 0) + (s.paymentBreakdown?.rocket || 0) + (s.paymentBreakdown?.nagad || 0) + (s.paymentBreakdown?.bank || 0);
    }, 0);
  ...
  const paymentBreakdown = useMemo(() => {
    ...
    return Object.entries(breakdown).filter(([, v]) => v > 0).map(([name, value]) => ({ name: name.toUpperCase(), value }));
  }, [sales]);

  const dailySales = useMemo(() => {
    const grouped = {};
    sales.forEach(s => {
      const date = new Date(s.createdAt).toLocaleDateString('en-BD', { month: 'short', day: 'numeric' });
      grouped[date] = (grouped[date] || 0) + (s.netTotal || 0);
    });
    return Object.entries(grouped).map(([date, total]) => ({ date, total })).slice(-15);
  }, [sales]);

  const COLORS = ['#dc2626', '#f59e0b', '#8b5cf6', '#ec4899', '#3b82f6', '#6b7280'];

  const statCards = [
    { label: 'Net Revenue', value: `৳${stats.totalRevenue.toLocaleString()}`, icon: DollarSign, color: 'text-green-600 dark:text-green-400', bg: 'bg-green-50 dark:bg-green-900/20' },
    { label: 'Retail Revenue (B2C)', value: `৳${stats.retailRevenue.toLocaleString()} (${stats.retailCount})`, icon: ShoppingCart, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-900/20' },
    { label: 'Wholesale Revenue (B2B)', value: `৳${stats.wholesaleRevenue.toLocaleString()} (${stats.wholesaleCount})`, icon: TrendingUp, color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-900/20' },
    { label: 'Net Collected Cash', value: `৳${stats.totalPaid.toLocaleString()}`, icon: TrendingUp, color: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-50 dark:bg-purple-900/20' },
    { label: 'Total Returns & Refunds', value: `৳${stats.totalReturned.toLocaleString()} (${stats.allReturnLogs.length} logs)`, icon: RotateCcw, color: 'text-red-600 dark:text-red-400', bg: 'bg-red-50 dark:bg-red-900/20' },
    { label: 'Pending Due', value: `৳${stats.totalDue.toLocaleString()}`, icon: BarChart3, color: 'text-red-600 dark:text-red-400', bg: 'bg-red-50 dark:bg-red-900/20' },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Sales &amp; Revenue Reports</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">Analyze performance across Retail (B2C) and Wholesale (B2B) sales</p>
        </div>
      </div>
        ...
        {statCards.map((card) => (
          <div key={card.label} className="bg-white dark:bg-[#111827] rounded-xl border border-gray-200 dark:border-gray-800 p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-medium text-gray-500 dark:text-gray-400 uppercase">{card.label}</span>
              <div className={`w-7 h-7 rounded-lg ${card.bg} flex items-center justify-center`}>
                <card.icon className={`w-3.5 h-3.5 ${card.color}`} />
              </div>
            </div>
            <div className={`text-lg font-bold ${card.color}`}>
              {isLoading ? <div className="h-6 w-16 bg-gray-200 dark:bg-gray-700 rounded animate-pulse" /> : card.value}
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-[#111827] rounded-xl border border-gray-200 dark:border-gray-800 p-6">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-4">Daily Sales Trend</h3>
          {isLoading ? (
        ...
        <div className="bg-white dark:bg-[#111827] rounded-xl border border-gray-200 dark:border-gray-800 p-6">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-4">Payment Methods</h3>
          {isLoading ? (
            <div className="h-64 bg-gray-200 dark:bg-gray-700 rounded animate-pulse" />
            ...
            <div className="flex items-center gap-6">
              <ResponsiveContainer width={150} height={150}>
                <PieChart>
                  <Pie data={paymentBreakdown} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={60}>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-800">
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">Payment</th>
              ...
              {isLoading ? (
                ...
                return (
                  <tr key={s._id} className="border-b border-gray-100 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/30">
                    <td className="px-4 py-3"><span className="text-sm font-mono text-gray-900 dark:text-gray-100">{s.invoiceNumber}</span></td>
                    <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{s.customerName}</td>
                    <td className="px-4 py-3 text-right text-sm font-medium text-gray-900 dark:text-gray-100">৳{s.netTotal?.toLocaleString()}</td>
                    <td className="px-4 py-3">
                      <div className="flex gap-1 flex-wrap">
                        {s.paymentBreakdown?.cash > 0 && <span className="text-[10px] px-1.5 py-0.5 rounded bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400">Cash</span>}
                        {s.paymentBreakdown?.bkash > 0 && <span className="text-[10px] px-1.5 py-0.5 rounded bg-pink-100 dark:bg-pink-900/20 text-pink-700 dark:text-pink-400">bKash</span>}
                        {s.paymentBreakdown?.dueAmount > 0 && <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400">Due</span>}
                      </div>
                    </td>
