      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 flex items-center gap-2">
            <Building2 className="w-6 h-6 text-blue-600 dark:text-blue-400" /> Branch & Outlet
            Management
          </h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Manage multi-branch shop locations and active outlet contexts
          </p>
        </div>
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2 px-3.5 py-2 bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 rounded-xl text-xs font-bold text-blue-700 dark:text-blue-300">
            ...
            <span>
              Plan Limit: {branches.length} / {maxBranches === 999 ? '∞' : maxBranches} (
              {tenantPlan})
            </span>
          </div>
          <button
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400">
                Your current{' '}
                <span className="font-bold text-amber-600 dark:text-amber-400">{tenantPlan}</span>{' '}
                subscription plan permits up to {maxBranches} outlet{maxBranches === 1 ? '' : 's'}.
                Upgrade to Pro or Enterprise for additional outlets.
              </div>
            </div>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          filtered.map((branch) => {
            ...
            return (
              <div
                key={branch._id}
                className={`${cardCls} p-5 hover:shadow-lg transition-all relative overflow-hidden flex flex-col justify-between`}
              >
                <div>
                  ...
                  >
                    {isActiveContext ? (
                      <>
                        ...
                        Currently Selected Active Outlet
                      </>
                    ) : (
                      <>
