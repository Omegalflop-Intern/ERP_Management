...
import { Plus, Search, Edit, Trash2, Tags, FolderOpen, Loader2, X } from 'lucide-react';
...
export default function Categories() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Deleted'); queryClient.invalidateQueries({ queryKey: ['catalog'] }); },
    ...
    <div className="space-y-6">
      <div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-5 rounded-2xl ${styled ? 'neu-card' : 'bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800'}`}>
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Manage product categories and brand names</p>
        </div>
      ...
      <div className="relative max-w-md">
        ...
        <input type="text" placeholder={`Search ${activeTab === 'CATEGORY' ? 'categories' : 'brands'}...`} value={search} onChange={(e) => setSearch(e.target.value)}
      ...
      {isLoading ? (
        ...
        <div className={`text-center py-16 ${styled ? 'neu-card rounded-2xl' : 'bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl'}`}>
          <FolderOpen className="w-12 h-12 text-gray-300 dark:text-gray-600 mx-auto mb-3" />
          <p className="text-gray-500 dark:text-gray-400 font-medium">No {activeTab === 'CATEGORY' ? 'categories' : 'brands'} found</p>
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Click "Add" to create one</p>
        </div>
      ) : (
        <div className={`overflow-hidden ${styled ? 'neu-card rounded-2xl' : 'bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl'}`}>
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item._id} className="border-b border-gray-100 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/30">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs font-semibold px-2 py-1 rounded-full ${item.type === 'CATEGORY' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' : 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400'}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button onClick={() => confirmDelete(`Delete "${item.name}"?`, () => deleteMutation.mutate(item._id))}
      ...
      {showForm && (
        <CatalogFormModal
          ...
          onSuccess={() => { setShowForm(false); setEditItem(null); queryClient.invalidateQueries({ queryKey: ['catalog'] }); }}
        />
      )}
    </div>
  );
...
function CatalogFormModal({ item, type, onClose, onSuccess }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { toast.success(item ? 'Updated' : 'Created'); onSuccess(); },
    onError: (e) => toast.error(e.response?.data?.message || 'Failed'),
  });
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className={`w-full max-w-sm ${styled ? 'neu-card rounded-2xl' : 'bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl'} shadow-xl`}>
        <div className={`px-6 py-4 flex items-center justify-between ${styled ? '' : 'border-b border-gray-200 dark:border-gray-800'}`}>
          <h3 className="font-bold text-gray-900 dark:text-gray-100">{item ? 'Edit' : 'Add'} {type === 'CATEGORY' ? 'Category' : 'Brand'}</h3>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate({ name }); }} className="p-6 space-y-4">
          <div>
            <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Name</label>
            <input required value={name} onChange={(e) => setName(e.target.value)} autoFocus className={inputCls} />
          </div>
