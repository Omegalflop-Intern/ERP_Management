      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Track every device by IMEI — full lifecycle passport
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {importMutation.isPending ? 'Importing...' : 'Import IMEI'}
          </button>
          <input
            ref={fileInputRef}
            type="file"
              ...
              {isLoading ? (
                ...
                units.map((u) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-xs font-semibold px-2 py-1 rounded-full ${u.status === 'Available' ? 'bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400' : u.status === 'Sold' ? 'bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300'}`}
                      >
                        {u.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right text-sm text-gray-700 dark:text-gray-300">
                      ৳{u.purchasePrice?.toLocaleString()}
                    </td>
                    <td className="px-4 py-3 text-right text-sm font-medium text-green-700 dark:text-green-400">
                      ৳{u.currentSellingPrice?.toLocaleString()}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => setViewPassport(u.imeiOrSerial)}
                          title="View Passport"
  ...
  const mutation = useMutation({
    mutationFn: async (data) =>
      api.post('/inventory', {
        ...
        currentSellingPrice: Number(data.currentSellingPrice) || Number(data.purchasePrice),
        warrantyMonths: Number(data.warrantyMonths),
      }),
    onSuccess: () => {
      toast.success('Stock inward successful');
      onSuccess();
    },
        ...
        >
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:border-red-500 font-mono"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm({ ...form, currentSellingPrice: e.target.value })}
                className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:border-red-500"
              />
            </div>
          </div>
...
function PassportModal({ imei, onClose }) {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(`/inventory/passport/${encodeURIComponent(imei)}`);
      return res.data?.data;
    },
  });
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 dark:bg-gray-900 rounded-xl p-4">
                ...
                <div className="text-sm text-gray-700 dark:text-gray-300 mt-1">
                  {data.productId?.brand} {data.productId?.name}
                </div>
                <div className="flex gap-2 mt-2">
                  <span
                    className={`text-xs font-semibold px-2 py-1 rounded-full ${data.status === 'Available' ? 'bg-green-100 text-green-700' : data.status === 'Sold' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'}`}
                  >
                    {data.status}
                  </span>
