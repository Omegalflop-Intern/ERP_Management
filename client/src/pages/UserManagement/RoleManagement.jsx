      ...
      >
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Create roles and assign page access permissions
          </p>
        </div>
        <button
          onClick={() => {
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {roles.map((role) => (
            <div
              key={role._id}
              className={`p-5 rounded-2xl space-y-3 ${styled ? 'neu-card' : 'bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800'}`}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center ${role.isSystem ? 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400' : 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400'}`}
                  >
                ...
                <div className="flex items-center gap-1">
                  ...
                  {!role.isSystem && (
                    <button
                      onClick={() =>
                        confirmDelete(`Delete "${role.name}" role?`, () =>
...
function RoleFormModal({ role, onClose, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div
        className={`w-full max-w-2xl max-h-[90vh] overflow-y-auto ${styled ? 'neu-card rounded-2xl' : 'bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl'} shadow-xl`}
      >
        <div
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                disabled={!!role}
                className={`${inputCls} ${role ? 'opacity-60' : ''}`}
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
              {Object.entries(PERMISSION_GROUPS).map(([group, perms]) => {
                ...
                return (
                  <div
                    key={group}
                    className={`p-3 rounded-xl ${styled ? 'neu-card-sm' : 'bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50'}`}
                  >
