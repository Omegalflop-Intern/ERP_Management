...
import { Plus, Search, Edit, Trash2, UserCog, Loader2, X, Eye, EyeOff, ShieldCheck, ShieldOff } from 'lucide-react';
...
export default function UserList() {
  ...
  const inputCls = styled ? 'neu-input w-full px-3 py-2 rounded-lg text-sm' : 'w-full px-3 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:border-red-500';
  const cardCls = styled ? 'neu-card rounded-2xl' : 'bg-white dark:bg-[#111827] rounded-xl border border-gray-200 dark:border-gray-800';
      ...
      <div className={`${cardCls} overflow-hidden`}>
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan={4} className="px-4 py-12 text-center"><Loader2 className="w-6 h-6 animate-spin text-red-500 mx-auto" /></td></tr>
              ) : users.length === 0 ? (
                <tr><td colSpan={4} className="px-4 py-12 text-center text-gray-500 dark:text-gray-400">
                  <UserCog className="w-12 h-12 mx-auto mb-3 opacity-50" /><p>No users found</p>
                </td></tr>
              ) : (
                users.map((u) => (
                  <tr key={u._id} className="border-b border-gray-100 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/30">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{u.fullName || u.username}</div>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        <span className={`text-xs font-semibold px-2 py-1 rounded-full ${u.isActive ? 'bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400'}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => toggleVerifyMutation.mutate(u._id)}
                          ...
                          {u.isVerified === false ? <ShieldCheck className="w-4 h-4" /> : <ShieldOff className="w-4 h-4" />}
                        </button>
                        <button onClick={() => { setEditUser(u); setShowForm(true); }}
                          className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500 hover:text-blue-600 transition-colors"><Edit className="w-4 h-4" /></button>
                        <button onClick={() => confirmDelete(`Delete user "${u.name}"?`, () => deleteMutation.mutate(u._id))}
                          className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500 hover:text-red-600 transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))
...
function UserFormModal({ user, onClose, onSuccess }) {
  const [form, setForm] = useState({
    ...
    password: '', role: user?.role?._id || user?.role || '', fullName: user?.fullName || '',
  });
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className={`w-full max-w-md ${styled ? 'neu-card rounded-2xl' : 'bg-white dark:bg-[#111827] rounded-2xl border border-gray-200 dark:border-gray-800'} shadow-xl`}>
        <div className={`px-6 py-4 flex items-center justify-between ${styled ? '' : 'border-b border-gray-200 dark:border-gray-800'}`}>
          <h3 className="font-bold text-gray-900 dark:text-gray-100">{user ? 'Edit User' : 'Create User'}</h3>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300"><X className="w-5 h-5" /></button>
        </div>
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form); }} className="p-6 space-y-3 text-sm">
          ...
          <div>
            ...
            <input value={form.phone} onChange={(e) => setForm({...form, phone: e.target.value})} className={`${inputCls} font-mono`} />
          </div>
          {!user && (
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Password *</label>
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} required minLength={6} value={form.password} onChange={(e) => setForm({...form, password: e.target.value})} className={`${inputCls} !pr-10`} />
