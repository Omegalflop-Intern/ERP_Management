...
export default function ResetPassword() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-gray-50 dark:bg-[#0b0f19]">
      ...
      <div className="absolute w-[600px] h-[600px] -top-48 -left-48 rounded-full blur-[120px] animate-drift pointer-events-none" style={{ background: wc.glow }} />
      <div className="absolute w-[500px] h-[500px] -bottom-40 -right-40 rounded-full blur-[110px] animate-drift pointer-events-none" style={{ background: wc.accent, animationDelay: '4s' }} />
      <div className="absolute w-[350px] h-[350px] top-1/4 left-1/3 rounded-full blur-[90px] animate-float pointer-events-none" style={{ background: wc.mid, animationDelay: '2s' }} />

      {BUBBLES.map((b) => (
        <div key={b.id} className="absolute rounded-full pointer-events-none" style={{ width: b.size, height: b.size, left: `${b.left}%`, bottom: '-20px', background: wc.foam, border: `1px solid ${wc.light}`, animation: `bubble-rise ${b.duration}s linear ${b.delay}s infinite` }} />
      ))}
      {FOAM_PARTICLES.map((f) => (
        <div key={`foam-${f.id}`} className="absolute pointer-events-none rounded-full" style={{ width: f.width, height: 3, left: `${f.left}%`, bottom: `${f.bottom}%`, background: `linear-gradient(90deg, transparent, ${wc.foam}, transparent)`, animation: `foam-slide ${f.duration}s linear ${f.delay}s infinite`, opacity: 0.5 }} />
      ))}

      <div className="absolute bottom-0 left-0 w-full pointer-events-none z-[1]" style={{ height: '35%' }}>
        <svg className="absolute bottom-0 w-[200%] h-full" viewBox="0 0 1440 320" preserveAspectRatio="none" style={{ animation: 'wave 18s linear infinite' }}>
          <path fill={wc.deep} d="M0,224L48,213.3C96,203,192,181,288,181.3C384,181,480,203,576,218.7C672,235,768,245,864,234.7C960,224,1056,192,1152,181.3C1248,171,1344,181,1392,186.7L1440,192L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z" />
        </svg>
        <svg className="absolute bottom-0 w-[200%] h-full" viewBox="0 0 1440 320" preserveAspectRatio="none" style={{ animation: 'wave 14s linear -3s infinite' }}>
          <path fill={wc.mid} d="M0,256L48,245.3C96,235,192,213,288,208C384,203,480,213,576,229.3C672,245,768,267,864,261.3C960,256,1056,224,1152,213.3C1248,203,1344,213,1392,218.7L1440,224L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z" />
        </svg>
        <svg className="absolute bottom-0 w-[200%] h-full" viewBox="0 0 1440 320" preserveAspectRatio="none" style={{ animation: 'wave 10s linear -1s infinite' }}>
          <path fill={wc.light} d="M0,288L48,277.3C96,267,192,245,288,234.7C384,224,480,224,576,234.7C672,245,768,267,864,261.3C960,256,1056,224,1152,213.3C1248,203,1344,213,1392,218.7L1440,224L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z" />
        </svg>
        <svg className="absolute bottom-0 w-[200%] h-[70%]" viewBox="0 0 1440 320" preserveAspectRatio="none" style={{ animation: 'wave 8s linear -2s infinite' }}>
          <path fill={wc.foam} d="M0,288L60,282.7C120,277,240,267,360,261.3C480,256,600,256,720,266.7C840,277,960,299,1080,293.3C1200,288,1320,256,1380,240L1440,224L1440,320L1380,320C1320,320,1200,320,1080,320C960,320,840,320,720,320C600,320,480,320,360,320C240,320,120,320,60,320L0,320Z" />
        </svg>
        <svg className="absolute bottom-0 w-[200%] h-[55%]" viewBox="0 0 1440 320" preserveAspectRatio="none" style={{ animation: 'wave 12s linear -4s infinite', opacity: 0.7 }}>
          <path fill={wc.accent} d="M0,298L48,293.3C96,288,192,277,288,272C384,267,480,267,576,277.3C672,288,768,309,864,304C960,299,1056,267,1152,256C1248,245,1344,256,1392,261.3L1440,267L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z" />
        </svg>
        <svg className="absolute bottom-0 w-[200%] h-[40%]" viewBox="0 0 1440 320" preserveAspectRatio="none" style={{ animation: 'wave 16s linear -6s infinite', opacity: 0.5 }}>
          <path fill={wc.glow} d="M0,304L60,298.7C120,293,240,283,360,277.3C480,272,600,272,720,282.7C840,293,960,315,1080,309.3C1200,304,1320,272,1380,256L1440,240L1440,320L1380,320C1320,320,1200,320,1080,320C960,320,840,320,720,320C600,320,480,320,360,320C240,320,120,320,60,320L0,320Z" />
        </svg>
        <svg className="absolute bottom-0 w-[200%] h-[25%]" viewBox="0 0 1440 320" preserveAspectRatio="none" style={{ animation: 'wave 9s linear -1s infinite', opacity: 0.35 }}>
          <path fill={wc.foam} d="M0,312L48,309.3C96,307,192,301,288,296C384,291,480,288,576,293.3C672,299,768,315,864,314.7C960,315,1056,299,1152,293.3C1248,288,1344,293,1392,296L1440,299L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z" />
        </svg>
      </div>
      ...
      <div className="relative z-10 w-full max-w-md mx-4">
        ...
        <div className={`rounded-2xl p-8 ${styled ? 'neu-card' : 'bg-white/80 dark:bg-white/[0.06] backdrop-blur-xl border border-white/40 dark:border-white/[0.08] shadow-2xl shadow-black/5 dark:shadow-black/30'}`}>
          {done ? (
              ...
              <div>
                ...
                {password && (
                  <div className="mt-2">
                    ...
                    <p className={`text-[11px] mt-1 font-medium ${strength >= 2 ? 'text-green-600' : strength >= 1 ? 'text-orange-500' : 'text-red-500'}`}>
