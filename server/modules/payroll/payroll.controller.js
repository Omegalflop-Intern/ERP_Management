...
export const getAllPayroll = async (req, res, next) => {
  try {
    ...
    const result = await payrollService.getAllPayroll(Number(page), Number(limit), month, year, status, tenantId);
...
export const processPayroll = async (req, res, next) => {
  try {
    ...
    const tenantId = req.user?.tenantId || null;
    const result = await payrollService.generatePayroll(month, year, employeeIds, tenantId, allowances, deductions);
