...
export const getMyNotifications = async (req, res, next) => {
  try {
    ...
    const result = await notificationService.getMyNotifications(req.user.userId, Number(page), Number(limit), unread === 'true');
...
export const markAsRead = async (req, res, next) => {
  try {
    const n = await notificationService.markAsRead(req.params.id, req.user.userId);
