...
export const getAllTransfers = async (req, res, next) => {
  try {
    ...
    const result = await stockService.getAllTransfers(Number(page), Number(limit), status);
...
export const getTransferById = async (req, res, next) => {
  try {
    const transfer = await stockService.getTransferById(req.params.id);
...
export const createTransfer = async (req, res, next) => {
  try {
    const transfer = await stockService.createTransfer(req.body, req.user?.username);
...
export const updateTransferStatus = async (req, res, next) => {
  try {
    const transfer = await stockService.updateTransferStatus(req.params.id, req.body.status, req.user?.username);
...
export const deleteTransfer = async (req, res, next) => {
  try {
    await stockService.deleteTransfer(req.params.id);
