...
export const logAction = async ({ userId, username, fullName, roleName, phone, tenantId, action, module, entityId, entityType, details, req }) => {
  try {
    const u = req?.user;
    const targetUserId = userId || u?.userId || u?.id || null;
    let computedTenantId = tenantId || u?.tenantId || null;

    if (targetUserId && !computedTenantId) {
      const userRow = await db('users').where({ id: targetUserId }).select('tenant_id', 'full_name', 'phone', 'username').first();
      if (userRow) {
